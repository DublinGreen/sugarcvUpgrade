...
class SiteController extends Controller
{
	...
	public function saveeditworkdetail($id, Request $request) {
		try{
			// if(Session::has('resume_id')){
				...
				// $resume_id = Session::get('resume_id');
				// $data = Resumeworkdetail::where(["id" => $id, "resume_id" => $resume_id])->update([
	...
	public function deleteWorkdetail(Request $request) {
		$id = $request->input('id');
		Resumeworkdetail::where("id",$id)->delete();
	...
	public function editResume($id) {
        ...
        return view('frontend.pages.resume',["resume" => $resume, "edit" => true]);
    }
	...
	public function createnewresume(Request $request){
		try{
			...
			if($request->has('id')){
				...
				$update = ResumeBasicDetail::where('id', $resume_id)->update($update);
			}elseif(Session::has('resume_id')){
				$resume_id = Session::get('resume_id');
		...
		try{
			if(Session::has('resume_id') || $request->has('id')){
				...
				$data  = Resumeeducationdetail::where('id',$id)->update([
					...
					'graduation_start_date' => $request->input('start_date'),
					'graduation_end_date' => $request->input('end_date'),
	...
	public function deleteEducation(Request $request) {
		$id = $request->input('id');
		Resumeeducationdetail::where("id",$id)->delete();
	...
	public function downloadDocument(Request $request){
		...
		if($format == 'pdf'){
			...
			$pdf = PDF::loadView($skin, compact('resume', 'waterMark'));

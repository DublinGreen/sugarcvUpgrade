...
$(function(){
    ...
    stop: function(event, ui) {
		...
		jQuery.ajax({
			url:"<?php echo route('order-workdetail') ?>",
			type:"POST",
			data:{sort:data},
  ...
  <main id="main">
   ...
   <section id="services" class="section-bg">
	 ...
	 <div class="row about-container">
        ...
        @foreach($summary as $sumry)
			<div class="col-lg-12 work-list" data-id="{{ $sumry->id }}">
				  <div class="para-item">
					 <div class="para-head">
						...
						<h5 class="para-title h5">{{$sumry->job_title}}</h5>
						<p class="para-s-title">{{$sumry->employer}}</p>
						<p class="para-s-title">{{$sumry->state}},{{$sumry->country}}</p>
						<p class="para-s-title">{{ date('M,Y', strtotime($sumry->start_date)) }} - {{ $sumry->is_current == '1' ? 'Current' : date('M,Y', strtotime($sumry->end_date)) }}</p>
					 </div>
					 <div class="para-info bottom-shadow">
						<ul>
						   {!!html_entity_decode($sumry->work_history)!!}
						</ul>
					 </div>
					 <div class="para-toolbar">
						@php
							$attr = isset($edit) ? [ 'id' => $sumry->id, 'edit' => $id ] : [ 'id' => $sumry->id ];
						@endphp
						<a id="iconEdit" href="{{ route('edit-work-detail', $attr )}}" class="" title="Edit"><i class="fa fa-pencil fs-16"></i></a>
						<a class="deleteHistory" tabindex="2" data-toggle="modal" data-target="#deleteWorkHistory" data-id="{{ $sumry->id }}" title="Delete"><i class="fa fa-trash"></i></a>
		  ...
		  @endforeach
			...
			@php
				$attr = isset($edit) ? [ 'edit' => $id ] : [ ];
			@endphp
		  <div class="col-lg-12" >
			  <a class="btn btn-block btn-block-add" href="{{route('work-detail', $attr)}}"id="btn_expr_addanother" type="button"><i class="fa fa-plus-circle icon-plus"></i>ADD ANOTHER POSITION</a>
		  </div>
		  <p>&nbsp;</p>
          <div class="col-lg-12" >
		  @if(isset($id))
			  <div class="" style="float:right"><a href="{{url('final-resume/'.$id)}}"  class="c-btn btn-success btn-lg" title="" >Next Education</a></div>
		  @else
			  <div class="" style="float:left"><a class="c-btn btn-default btn-back" href="{{route('create-resume')}}">Back</a></div>
			<div class="" style="float:right"><a href="{{url('education-detail')}}"  class="c-btn btn-success btn-lg" title="" >Next Education</a></div>
		  @endif
	...
	<div class="custom-modal modal fade" id="deleteWorkHistory" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
	  ...
	  <form action="{{ route('delete-workdetail') }}" method="post" >
		...
		@endif
		<input type="hidden" id="hiddenId" name="work_id" />
